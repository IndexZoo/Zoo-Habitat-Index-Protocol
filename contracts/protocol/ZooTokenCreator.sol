 ...
 */
contract ZooTokenCreator is Ownable {
    using AddressArrayUtils for address[];

    /* ============ Events ============ */

    event ZooTokenCreated(address indexed _zooToken, address _manager, string _name, string _symbol);
        ...
        returns (address)
    {
        ...
        ZooToken zooToken = new ZooToken(
            _components,
            _units,
            _modules,
            controller,
        ...
        controller.addSet(address(zooToken));

        emit ZooTokenCreated(address(zooToken), _manager, _name, _symbol);

        return address(zooToken);
    }
}
