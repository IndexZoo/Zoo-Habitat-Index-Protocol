 ...
 */
contract ZooStreamingFeeModule is ModuleBase, ReentrancyGuard {
    ...
    mapping(IZooToken => FeeState) public feeStates;
     ...
     */
    function accrueFee(IZooToken _setToken) public nonReentrant onlyValidAndInitializedSet(_setToken) {
        uint256 managerFee;
        uint256 protocolFee;
    ...
    function removeModule() external override {
        delete feeStates[IZooToken(msg.sender)];
    }
     ...
     */
    function updateFeeRecipient(IZooToken _setToken, address _newFeeRecipient)
        external
        onlySetManager(_setToken, msg.sender)
        onlyValidAndInitializedSet(_setToken)
    {
     ...
     */
    function getFee(IZooToken _setToken) external view returns (uint256) {
     ...
     */
    function _calculateStreamingFee(IZooToken _setToken) internal view returns(uint256) {
     ...
     */
    function _mintManagerAndProtocolFee(IZooToken _setToken, uint256 _feeQuantity) internal returns (uint256, uint256) {
    ...
    function _feeRecipient(IZooToken _set) internal view returns (address) {
        return feeStates[_set].feeRecipient;
    }

    function _lastStreamingFeeTimestamp(IZooToken _set) internal view returns (uint256) {
        return feeStates[_set].lastStreamingFeeTimestamp;
    }

    function _maxStreamingFeePercentage(IZooToken _set) internal view returns (uint256) {
        return feeStates[_set].maxStreamingFeePercentage;
    }

    function _streamingFeePercentage(IZooToken _set) internal view returns (uint256) {
