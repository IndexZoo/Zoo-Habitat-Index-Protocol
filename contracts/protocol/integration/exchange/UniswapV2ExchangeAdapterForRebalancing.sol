 ...
 *
 */
contract UniswapV2ExchangeAdapterForRebalancing {
    ...
    // Uniswap router function string to estimate amount out for a given amount in
    string internal constant GET_AMOUNTS_OUT = "getAmountsOut(uint256,address[])"; 
    // Uniswap router function string to estimate amount in for a given amount out
    string internal constant GET_AMOUNTS_IN = "getAmountsIn(uint256,address[])";
     ...
     *
     * @param  _sourceToken              Source token of the estimation
     * @param _otherToken          Destination token of the estimation 
     * @param  _quantity                 Fixed/Max amount of token to exchange in 
     * @param  _direction                DIRECTION_IN: call getAmountsIn - DIRECTION_OUT: call getAmountsOut
     *
     * @return address                   Target contract address
     * @return uint256                   Call value
     * @return bytes                     Trade calldata
     */
    function getAmountsCalldata(
        ...
        returns (address, uint256, bytes memory)
    {
        ...
        path[0] = _direction == DIRECTION_OUT? _sourceToken : _otherToken;
        path[1] = _direction == DIRECTION_OUT? _otherToken : _sourceToken;
        bytes memory callData = abi.encodeWithSignature(
            _direction == DIRECTION_OUT ? GET_AMOUNTS_OUT : GET_AMOUNTS_IN,
            _quantity, 
            path 
        );
